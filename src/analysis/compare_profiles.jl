using JLD2
using Statistics
using Plots

gr()

water_depth = 100.0


function newest_output_run(basedir="output")
    dirs = [joinpath(basedir, d) for d in readdir(basedir) if isdir(joinpath(basedir, d))]
    isempty(dirs) && error("No experiment folders found in output/")
    newest = dirs[argmax(stat(d).mtime for d in dirs)]
    return newest
end

outdir = newest_output_run()
println("Using outdir = ", outdir)


files = filter(f -> endswith(f, ".jld2"), readdir(outdir, join=true))
sort!(files)

if length(files) < 2
    error("Need at least 2 snapshot files in $(outdir) to compare.")
end

first_file = files[1]
last_file  = files[end]

d0 = load(first_file)
d1 = load(last_file)

T0 = d0["T"]; t0 = d0["t"]
T1 = d1["T"]; t1 = d1["t"]

println("Max |ΔT| pointwise = ", maximum(abs.(T1 .- T0)))

# Horizontal mean profiles
Tz0 = dropdims(mean(T0, dims=(1,2)), dims=(1,2))
Tz1 = dropdims(mean(T1, dims=(1,2)), dims=(1,2))

# Depth axis
Nz = length(Tz0)
z = range(0, stop=water_depth, length=Nz)


# plot comparison

p = plot(
    Tz0, z,
    xlabel="Temperature",
    ylabel="Depth (m)",
    title="Mean Temperature Profile\nfirst vs last snapshot",
    yflip=true,
    label="first (t=$(round(t0)) s)"
)

plot!(p, Tz1, z,
      label="last (t=$(round(t1)) s)")

display(p)

diff_norm = maximum(abs.(Tz1 .- Tz0))

println("\nExperiment directory: ", outdir)
println("First snapshot: ", first_file)
println("Last snapshot:  ", last_file)
println("Max |ΔTz| between first and last = ", diff_norm)

println("\nPress Enter to close...")
readline()