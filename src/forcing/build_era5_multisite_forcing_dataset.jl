...
# walks data/ERA5/ and stacks every site's era5 vars + bulk formula targets
# into one big (X, Y) matrix for training. v1 version, 8 features, no d2m.
# v2 is in build_era5_multisite_forcing_dataset_v2.jl
...
# extracts lat/lon from filenames like "lat30lon-50ERA5.nc"
...
function build_multisite_dataset(;
        era5_dir=joinpath(@__DIR__, "..", "..", "data", "ERA5"),
    ...
    for f in files
        ...
        # sanity check all vars are the same length
        ...
        QT     = net_surface_heat_flux(data.t2m, data.sst, data.u10, data.v10, data.ssrd, data.strd)

        # 8 features: 6 raw + lat/lon so the net knows where it is
        X = hcat(data.u10, data.v10, data.t2m, data.sst, data.ssrd, data.strd,
                 fill(lat, n), fill(lon, n))
        Y = hcat(τx, τy, QT)

        # toss any row where any feature/target is NaN or Inf
    ...
    # print summary stats so i can eyeball if anything looks broken
