...
# v2 no-QT sim. same as ml_forced_run_v2.jl but without the heat flux BC.
# baseline for showing how much the QT actually changes things.
...
out_file = "output/era5/ml_forced_$(sim_days)day_v2_noqt_$(site).jld2"
tmp_file = "output/era5/ml_forced_$(sim_days)day_v2_noqt_$(site)_checkpoint.jld2"
...
println("[v2 noQT] site=$site  lat=$site_lat lon=$site_lon")

# 13-input mlp inference, but only keep τx/τy (skip QT)
...
for i in 1:Nt
    ...
    τx[i] = y[1]
    τy[i] = y[2]
    # y[3] is QT but this run doesn't use it
end
...
# sign-negated for oceananigans convention, same as everywhere else
...
# no T boundary condition — T will evolve from init by diffusion/advection only
...
println("Running $(sim_days)-day v2 no-QT run at $(site)")
run!(simulation)
...
println("$(sim_days)-day v2 no-QT run completed at $(site)")
